<template>
  <main class="p-shop">
    ...
    <div class="u-container">
      <UiCBreadcrumbs type="brand" :url="currentBrand" :items="breadcrumbs" />
      <div class="p-shop__flex">
        <div class="p-shop__sidebar">
          <LayoutLSidebar :slug="currentBrand" :items="sidebarData" />
        </div>
          ...
          <section class="l-shop">
            ...
            <div v-if="description" class="l-shop__text js-read-more">
              <div
                ...
                v-html="description"
              ></div>
              <a class="u-read-more js-read-more-btn" :class="{ 'is-open': isReadMoreOpen }" @click="isReadMoreOpen = !isReadMoreOpen">
                {{ te('Read completely') ? t('Read completely') : 'Read completely' }}
              </a>
            </div>
            <LayoutLFilterBlock
              :slug="currentBrand"
...
const currentBrand = computed(() => {
  const brand = route.params.brandName
  if (Array.isArray(brand)) {
    const filtered = brand.filter(Boolean)
    return filtered[0] || ''
  }
  return brand || ''
})
...
const { data: pageData, pending } = await useAsyncData(
  `brand:${currentBrand.value}:${locale.value}`,
  async ( {signal} ) => {
    const slug = currentBrand.value
